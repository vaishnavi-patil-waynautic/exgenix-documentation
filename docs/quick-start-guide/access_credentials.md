---
id: access_credentials
title: 'Access Credentials'
sidebar_position: 1
hide_table_of_contents: true
sidebar_label: Access Credentials
slug: /quick-start-guide/access_credentials
description : 'For exgenix | AI QA Agent | JIRA | ADO'
---


# Access Credentials

---

| Platform | Access Link | Username | Password | Notes |
|-----------|--------------|-----------|-----------|-------|
| **Exgenix Platform** | https://app.exgenix.waynautic.com/ | `testuser1@yahoo.com` and `testuser2@yahoo.com` | `Test@123` | Preloaded with **critical APIs and collections** |
| **AI QA Agent** | https://app.aiqa.exgenix.waynautic.com/ | `testuser1@yahoo.com` | `Test@123` |Preloaded with **Exchange QA Project** |
| **JIRA** | https://id.atlassian.com/login | `testuserwaynautic@yahoo.com` | `Waynautic@2025` | Integrated with **Exgenix test traceability**. Use Project `AI Agent Testing` |
| **ADO** | https://azure.microsoft.com/en-us/products/devops | `testuserwaynautic@yahoo.com` | `< Get Code from Yahoo mail to sign in >` | Integrated with **Exgenix test traceability** |
| **Yahool Mail** | https://mail.yahoo.com | `testuserwaynautic@yahoo.com` | `Waynautic@2025` | Email access for notifications |
