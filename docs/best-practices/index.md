---
title: Best Practices
slug: /best-practices
hide_table_of_contents: true
id: best-practices
---


# Best Practices

This section outlines **recommended best practices** for designing, importing, executing, and analyzing test scenarios in Exgenix Simulator.  
Following these guidelines ensures **consistency, reusability, accuracy, and easier troubleshooting** across test cycles.

---

## Table of Contents
- [Naming Convention](best-practices/naming-convention)
- [AI Generated Testcases](best-practices/ai-generated-test-cases)
- [Datasheet Best Practices](best-practices/datasheet-best-practices)
- [Test Case Import Functionality](best-practices/test-case-import-functionality)
- [Test Report Analysis & Troubleshooting](best-practices/troubleshooting)
- [Special Cases](best-practices/special-cases)

---

